...
XFDetector::XFDetector(int _top_k, float _detection_threshold, bool use_cuda)
  ...
  std::cout << "Model converted to half precision (FP16)" << std::endl;
...
void XFDetector::match(torch::Tensor& feats1,
                       torch::Tensor& feats2,
                       torch::Tensor& idx0,
                       torch::Tensor& idx1,
                       float _min_cossim) {
...
void XFDetector::match_xfeat(cv::Mat& img1,
                             cv::Mat& img2,
                             cv::Mat& mkpts_0,
                             cv::Mat& mkpts_1) {
...
torch::Tensor XFDetector::extractDenseFeatures(torch::Tensor& x) {
  torch::Device device(device_type);
  x = x.to(device);
  x = x.to(torch::kHalf);
...
torch::Tensor XFDetector::NMS(torch::Tensor& x,
                              float threshold,
                              int kernel_size) {
  int B = x.size(0);
  int H = x.size(2);
