...
class XFDetector {
 public:
  XFDetector(int _top_k = 4096,
             float _detection_threshold = 0.05,
             bool use_cuda = true);
  void detectAndCompute(torch::Tensor &x,
                        std::unordered_map<std::string, torch::Tensor> &result);
  void match(torch::Tensor &feats1,
             torch::Tensor &feats2,
             torch::Tensor &idx0,
             torch::Tensor &idx1,
             float _min_cossim = -1.0);
  void match_xfeat(cv::Mat &img1,
                   cv::Mat &img2,
                   cv::Mat &mkpts_0,
                   cv::Mat &mkpts_1);
  torch::Tensor extractDenseFeatures(torch::Tensor &x);
 ...
 private:
  ...
  torch::Tensor NMS(torch::Tensor &x,
